...
//매개변수 : 외부에서 들어온 데이터를 저정한 변수
//인자 : 외부에서 전달한 데이터(값,변수)

// ---

//함수 표현식 정의
//두 숫자를 매개변수로 받아서 뺀 값을 변환하는 함수
...
//전달받은 매개변수가 음수,0양수 인지를 판단해서 출력
const determaine = function (a) {
  if (a < 0) {
    console.log("음수");
  } else if (a === 0) {
    console.log("0");
...
// 1. function 키워드를 지운다
// 2. 함수명을 지운다
// 3. 매개변수와 함수 몸체(코드블럭)를 =>연결한다
// 4.
const determaineArrow = (a) => {
  if (a < 0) {
    console.log("음수");
  } else if (a === 0) {
    console.log("0");
...
//화살표 함수 표현 코드 작성
const evenOddd = function (number1) {
  if (number1 % 2 === 0) {
    console.log("짝수")
    return true
  } else {
