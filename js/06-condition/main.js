//if 조건문의 기본 구조

/* 
if(조건식){
  //조건식이 참이면 실행되는 코드 영역
}
*/

if (5 > 1) {
  console.log("5는 1보다 크다");
}

/*if ~ else if조건문 기본 구조
if(조건식){
  if의 조건식이 참일때 실행
}
else if(조건식'){
  조건식'는 조건식이 거짓일때 평가되고, 조건식'가 참이면 실행
}
else if(조건식''){
  조건식''는 조건식'가 거짓일떄 평가
}
*/

if (5 > 10) {
  console.log("5는 10보다 다");
} else if (5 > 7) {
  console.log("5는 7보다 크다");
} else if (5 > 5) {
  console.log("5는5보다 크다");
} else if (5 > 3) {
  console.log("5는3보다 크다");
}

/*if ~ else if~ else 조건문 기본 구조
if(조건식){
}
else if(조건식'){
}
else{
  else는 조건식이 없다.
  대신 위의 모든 조건식이 거짓(false)이면 실행
  else 아래에는 또 다른 else if나 else가 존재할 수 없다.
  }
---
if(조건식){
}else{
  조건식이 거짓이면 실행
  }
*/
//-------------------------------------------------------------------------
//변수 number를 선언하고, 숫자 1을 할당
let number = 1;

//number 가 0보다 크면 "양수"를 콘솔에 출력
//number가 0보다 큰게 아니라면 "음수"를 콘솔에 출력
//if ~else 문 구현

if (number > 0) {
  console.log("양수");
} else {
  console.log("음수");
}

if (number > 0) {
  console.log("양수");
} else if (number === 0) {
  console.log("0");
} else {
  console.log("음수");
}
//-------------------------------------------------------------------------
//변수 number2를 선언하고, 숫자0을 할당
let number2 = 0;
//만약에 변수 number2 가 0보다 크면, "양수" 출력
//만약에 변수 number2가 0보다 작으면, "음수" 출력
//그런데 모두 아니라면, "0"출력
if (number2 > 0) {
  console.log("양수");
} else if (number2 < 0) {
  console.log("음수");
} else {
  console.log("0");
}
//---------------------------------------------------------------------------------
//중첩 조건문
//조건문 내부에 조건문을 중첩하는 조건문

//변수 age를 선언하고 , 숫자 25를 할당한다
//변수 isStudent를 선언하고 , 불리언 true를 할당한다
let age = 25;
let isStudent = true;
//1. 변수 age가 20 이상이라면
//1-1 변수 isStudent가 true라면
//1-2 변수isStuednt가 true가 아니라면

//2. 변수 age가 20 이상이 아니라면
//2-1 변수 isStudent가 true라면
//2-2 변수isStuednt가 true가 아니라면
if (age >= 20) {
  if (isStudent === true) {
    console.log("성인 학생입니다");
  } else {
    console.log("성인입니다");
  }
} else {
  if (isStudent === true) {
    console.log("미성년 학생입니다");
  } else {
    console.log("미성년 입니다");
  }
}

//-----------------------------------------------------------------------------------

// 변수 score를 선언하고, 숫자 75를 할당한다
// 만약 변수 socre가 90 이상이라면
// 그런데 만약 변수 score가 90 미만 그리고(&&), 80 이상이라면
// 그런데 만약 변수 score가 80 미만 그리고(&&), 70 이상이라면
// 그런데 만약 변수 score가 70 미만 그리고(&&), 60 이상이라면
// 그런데 모두 아니라면
//if,else if *3,else
let score = 75;
if (score >= 90) {
  console.log("score는 90이상이다");
} else if (score < 90 && score >= 80) {
  console.log("score는 90미만 80이상이다");
} else if (score < 80 && score >= 70) {
  console.log("score는 80미안 70이상이다");
} else if (score < 70 && score >= 60) {
  console.log("score는 70미만 60이상이다");
} else {
  console.log("score는 60 미만이다");
}

//심힝 얀신지 표현식
//표현식: 데이터를 생성하는 코드

//조건식은 무조건(true)/ 가짓(false) 데이터 중 하나를 생성
//조건식 ? 조건식 평가 결과가 참일 때  데이터 : 조건식 평가 결과가 거짓일 떼 데이터

const message = 2 > 1 ? "2는 1보다 크다" : "2는 1보다 크지 않다";
console.log(message);

let isLoggined = true;

const user = isLoggined === true ? "회원 사용자 화면" : "비 회원 사용자 화면";
